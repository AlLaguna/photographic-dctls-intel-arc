...
__CONSTANT__ struct CineonColor cineon_colors[] = {
    { -8.0f, 0.01f, 0.01f, 0.01f }, { -7.0f, 0.05f, 0.05f, 0.05f }, { -6.0f, 0.10f, 0.10f, 0.10f }, { -5.0f, 0.40f, 0.25f, 0.60f }, 
    { -4.0f, 0.20f, 0.45f, 0.70f }, { -3.0f, 0.40f, 0.60f, 0.95f }, { -2.0f, 0.40f, 0.60f, 0.25f }, { -1.0f, 0.60f, 0.90f, 0.55f },
    { 0.0f, 0.50f, 0.50f, 0.50f },  { 1.0f, 1.00f, 0.95f, 0.25f },  { 2.0f, 0.90f, 0.50f, 0.25f },  { 3.0f, 0.90f, 0.60f, 0.25f }, 
    { 4.0f, 0.90f, 0.30f, 0.20f },  { 5.0f, 0.90f, 0.35f, 0.30f },  { 6.0f, 0.90f, 0.90f, 0.90f },  { 7.0f, 0.95f, 0.95f, 0.95f },
    { 8.0f, 0.99f, 0.99f, 0.99f }
};
...
CineonCurve_reverse_cineon(struct CineonCurve cv, float3 rgb, float3 scale, float3 dmin)
{
    ...
    rgb = dmin / max_f3(pow10_f3(scaled), 1e-7f); 
    return rgb;
}
...
CineonCurve_lin_cineon(struct CineonCurve cv, float3 rgb)
{
    ...
    // log_f está protegido en PD-Common.h
    float r = offset + log_f(max_f((rgb.x - 1.0f) * gain + 1.0f, 1e-7f)) / (cv.bitdepth * log_f(10.0f) * scale); 
    float g = offset + log_f(max_f((rgb.y - 1.0f) * gain + 1.0f, 1e-7f)) / (cv.bitdepth * log_f(10.0f) * scale); 
    float b = offset + log_f(max_f((rgb.z - 1.0f) * gain + 1.0f, 1e-7f)) / (cv.bitdepth * log_f(10.0f) * scale); 
...
CineonCurve_cineon_lin(struct CineonCurve cv, float3 rgb)
{
    ...
    float diff = max_f(pow_f(10.0f, cv.white * scale) - black, 1e-7f); 
